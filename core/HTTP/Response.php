  ...
  class Response implements ResponseInterface
  {
    ...
    public function http_response_code(int $code = NULL, string $msj = NULL) 
    {
      if ($code !== NULL) {
        ...
        $protocol = (isset($this->request->serverProtocol) ? $this->request->serverProtocol : 'HTTP/1.0');

        header($protocol . ' ' . $code . ' ' . $text. $msj);
    ...
    protected function content_type(string $type) 
    {
      ...
      switch($type)
      {
        case 'json': $contentType = 'Content-Type: application/json'; break;
        case 'html': $contentType = 'Content-Type: html/text'; break;
      }
